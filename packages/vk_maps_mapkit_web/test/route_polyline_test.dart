...
void main() {
  group('декодирование настоящего маршрута', () {
    // Этот тест имеет смысл гонять и в браузере:
    ...
    test('все точки остаются в пределах маршрута', () {
      final List<VkLatLon> points = VkWebPolyline.decode(_moscowRoute);
      expect(points, hasLength(334));

      for (final VkLatLon point in points) {
        expect(
          point.latitude,
          inInclusiveRange(55.79, 55.97),
          reason: 'широта уехала за пределы маршрута',
        );
        expect(
          point.longitude,
          inInclusiveRange(37.39, 37.55),
          reason: 'долгота уехала за пределы маршрута',
        );
      }
    });
    ...
    test('ломаная превращается в GeoJSON той же длины', () {
      final String geoJson = VkWebPolyline.toGeoJson(_moscowRoute);
      expect(geoJson, contains('LineString'));
      expect('['.allMatches(geoJson).length, greaterThan(334));
    });
  });
}
