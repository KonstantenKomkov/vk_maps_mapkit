...
**Оба спорных решения приняты** 8 сентября 2026 и записаны в [`../docs/design-decisions.md`](../docs/design-decisions.md):
Pigeon-контракт остаётся общим в `platform_interface` (Р-2), нативный SDK подключается как `implementation` (Р-3).
...
2. Отправить письмо в `support.maps@lists.vk.team`. Готовый текст и таблица для фиксации переписки —
   ...
   и какое поколение считать актуальным; (3) кластеризация и источник для ломаных в нативном SDK; (4) требования к атрибуции
   и логотипу на карте.
...
**Сложность:** L.
**Зависимость:** этап 1.
**Статус:** выполнен 8 сентября 2026.
...
6. Раскладка пакета — как в `google_maps_flutter_platform_interface`: `src/types` (модели), `src/events` (события),
   `src/platform_interface` (абстракция), `src/method_channel` (реализация по умолчанию). Контракт — общий на обе
   платформы (решение Р-2).
...
**Статус:** код написан 8 сентября 2026 по документированному SDK (решение Р-8); сборка и живая проверка заблокированы недоступностью артефакта.
...
**Статус:** код написан 8 сентября 2026; живая проверка — этап 8.
...
6. **Поддержка SPM (Swift Package Manager).** Плагин публикуется в двух режимах подключения одновременно:
   CocoaPods (`.podspec`) и SPM (`Package.swift`). Раскладка, которую ищет Flutter:
...
**Статус:** выполнен 8 сентября 2026 (живая проверка — этап 8).
...
6. Инициализация SDK: `VkMaps.init(apiKey:, locale:, options:)` до первой карты, идемпотентная — повторный вызов
   ...
   **Логотип VK скрывать нельзя** (решение Р-7): в API отдаём только `logoAlignment` и `logoInsets`, параметра
   видимости нет и появиться не должно.
...
**Статус:** приложение и тесты готовы 8 сентября 2026; живая проверка не начата — нет ключа доступа и Android-артефакта.
...
**Сложность:** M.
**Зависимость:** этап 8.
**Статус:** документы написаны 8 сентября 2026; публикация ждёт живой проверки.
...
## 13a. Этап 10 — web поверх `MMR GL JS`

Решение Р-10: web входит в 0.1.0. Мост в натив здесь не нужен — пакет реализует `VkMapsPlatform` целиком и
вызывает библиотеку на странице через `dart:js_interop`.

### Задачи

1. ~~Пакет `vk_maps_mapkit_web`: `implements: vk_maps_mapkit`, `pluginClass: VkMapsWeb`~~ — сделано.
2. ~~Загрузчик библиотеки: скрипт и стили подключаются сами, адрес и версия настраиваются
   (`VkMapsSdkLoader`)~~ — сделано.
3. ~~Карта в `HtmlElementView`, пересчёт размера через `ResizeObserver`~~ — сделано.
4. ~~Камера, границы, обе проекции, события карты и камеры~~ — сделано.
5. ~~Маркеры одним слоем `symbol` (решение Р-12), касание через `queryRenderedFeatures`~~ — сделано.
6. ~~Стили, источники, слои, картинки; состояние хранится в плагине и переживает смену стиля (решение
   Р-13)~~ — сделано.
7. ~~Индикатор пользователя и режимы следования на стороне плагина~~ — сделано.
8. ~~Логотип VK: `LogoControl`, угол и отступы~~ — сделано.
9. Живая проверка в браузере с настоящим ключом — блокирована тем же ключом, что и мобильные платформы.

### Критерии готовности

- `flutter build web` в примере проходит, `VkMapsWeb` попадает в регистрант плагинов;
- различия web записаны в `docs/platform-matrix.md`, а расхождения документации со сборкой — в
  `docs/js-sdk-coverage.md`;
- чек-лист живой проверки содержит колонку Web и раздел «Только web».
...
- Приложение на Flutter показывает карту VK Карт на Android, iOS и web одной строкой `VkMap(...)`: на мобильных —
  через нативный SDK, на web — через `MMR GL JS`.
- Камера, маркеры, стили, GeoJSON/polyline-слои, изображения, режим следования и события покрыты Dart-API и
  проверены вживую на всех трёх платформах.
- Все REST-сервисы VK Карт доступны из чистого Dart с типизированными моделями и фикстурными тестами.
- Шесть пакетов опубликованы на pub.dev, CI зелёный, документация по-русски и по-английски.
- Известные ограничения (нет нативной кластеризации, доступность Android-артефактов, различия web из
  `docs/platform-matrix.md`) зафиксированы явно.
...
- [~] Этап 0 — решения приняты, письмо готово к отправке; ключ и артефакт за вами
- [x] Этап 1 — скелет монорепозитория
- [x] Этап 2 — `platform_interface` и Pigeon-контракт
- [~] Этап 3 — Android (код написан, сборка заблокирована артефактом)
- [x] Этап 4 — iOS (собирается, запускается на симуляторе)
- [x] Этап 5 — Dart-фасад
- [x] Этап 6 — стили, источники, слои, изображения
- [x] Этап 7 — `vk_maps_api`
- [x] Этап 10 — web (`vk_maps_mapkit_web`): пакет написан, пример собирается, карта создаётся в браузере
- [~] Этап 8 — example готов, живая проверка ждёт ключа
- [~] Этап 9 — документы готовы, публикация ждёт живой проверки

---

**Ход работ:** 8 сентября 2026 выполнены этапы 1, 2, 4, 5, 6, 7; этап 3 написан, но не собран; этапы 8 и 9
готовы настолько, насколько позволяют внешние блокеры (ключ доступа и Android-артефакт). 9 сентября 2026 добавлен
этап 10 — web-платформа (решение Р-10): пакет `vk_maps_mapkit_web` поверх `MMR GL JS`, пример собирается,
карта создаётся в браузере. Текущее состояние — [`../docs/status.md`](../docs/status.md).
