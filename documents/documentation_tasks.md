...
- **Пакет:** документация
- **Статус:** изучена — охват сервисов сверен с `vk_maps_api`, поддержанное перечислено в README
...
## JavaScript SDK — эталон поверхности API и платформа web

С 9 сентября 2026 эти страницы описывают не только эталон полноты Dart-API, но и рабочую платформу:
web поддерживается пакетом `vk_maps_mapkit_web` (решение Р-10).
...
- **Пакет:** —
- **Статус:** внедрена — web поддерживается (решение Р-10); модель JS SDK легла в основу `vk_maps_mapkit_web`
...
- **Внедрить:** Подключение библиотеки в `VkMapsSdkLoader`: скрипт и стили добавляются на страницу сами.
- **Пакет:** —
- **Статус:** внедрена — `VkMapsSdkLoader` грузит `mmr-gl.js` и `mmr-gl.css`, адрес и версия настраиваются
...
- **Статус:** изучена — контракт и контроллер сверены со списком методов и событий `Map`
...
- **Статус:** изучена — сверка 111 методов с Dart-API в [`../docs/js-sdk-coverage.md`](../docs/js-sdk-coverage.md)
...
- **Пакет:** platform_interface + фасад
- **Статус:** изучена — 48 событий сверены, поддержанные заведены в контракт; сводка в [`../docs/js-sdk-coverage.md`](../docs/js-sdk-coverage.md)
...
- **Пакет:** фасад + натив
- **Статус:** внедрена — флаги жестов в конфигурации карты
...
- **Пакет:** фасад
- **Статус:** внедрена частично — маркеры есть; попапы в нативном iOS SDK отсутствуют, контролы задаются свойствами карты
...
- **Пакет:** фасад (стили)
- **Статус:** внедрена — источники GeoJSON и ломаной; Video/Canvas отмечены как web-only в platform-matrix
...
- **Пакет:** фасад + example
- **Статус:** внедрена — `VkGeoJson` собирает точки, ломаные, полигоны и круги
...
- **Пакет:** фасад
- **Статус:** внедрена — `VkClusterizer`: жадная кластеризация на Dart с индексом по сетке
...
- **Пакет:** фасад
- **Статус:** изучена — атрибуция обеспечивается логотипом SDK, скрыть его нельзя (Р-7)
...
- **Пакет:** —
- **Статус:** не требуется — интеграция с React
...
- **Статус:** внедрена — пакет `vk_maps_mapkit_android` (решение Р-8); сборка заблокирована недоступностью артефакта
...
- **Статус:** внедрена — пакет `vk_maps_mapkit_ios` поверх нативного SDK
...
- **Статус:** внедрена — `VkPredefinedStyle` со значениями из DocC; на Android набор беднее, отмечено в матрице
